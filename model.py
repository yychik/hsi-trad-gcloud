...
def build_inputs(num_steps=1, n_features=74, n_class=3):
    # ===========================================================================================
    ...
    inputs = tf.placeholder(tf.float32, shape=(None, num_steps, n_features), name='inputs')
    targets = tf.placeholder(tf.int32, shape=(None, num_steps, n_class), name='labels')
    ...
    def __init__(self, n_class=3, batch_size=128, num_steps=10, gru_size=50, dense_size=50, num_layers=2,
        ...
        with tf.name_scope("accuracy"):
            correct_predictions = tf.equal(tf.argmax(self.prediction, 1),
                                           tf.argmax(tf.reshape(self.targets, self.prediction.get_shape()), 1))
